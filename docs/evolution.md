...
- **Canonical SOUL document** at `docs/soul.md`. The desktop loads it at build time as the system prompt for every reasoning call. `identity/soul.js` is a one-line bridge — identity lives in the repository, not in code, and not in the model. A webpack rule in `craco.config.js` allows `.md` to be imported as raw text.
