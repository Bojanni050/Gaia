...
**How it was done:** built `gaia-web:cutover` from `/root/gaia-web`, sanity-tested it standalone on a spare port (`127.0.0.1:8091`) joined to `gaia_net` — confirmed `/`, `/api/hermes/v1/models`, `/api/hindsight/health`, `/api/cognition/health` all `200` — before touching the live container. Swapped with a fast rollback path: old image retagged `gaia-web:before-cutover`, old container renamed `gaia-web-old` (stopped, not removed) rather than deleted, new image promoted to `gaia-web:latest`, new container started under the original name/port/network/restart-policy. Both the old container and the old image are still present on the VPS as of this writing — intentionally, as a rollback safety net; clean up (`docker rm gaia-web-old`, `docker rmi gaia-web:before-cutover`) once the new path has proven stable for a while.
