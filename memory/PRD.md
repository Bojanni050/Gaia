...
- `formPattern`/`queryPatterns` remain unimplemented (throw, per the base class) — Hindsight's nearest equivalent is "mental models," not yet wired. Hypotheses (architecture.md §6.1) are also not yet wired — Hindsight has no native hypothesis object; that would need to be built on top of its existing primitives, deferred to a later pass.
